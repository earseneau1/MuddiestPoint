...
// Generate anonymous session token (privacy-preserving)
...
export const addAnonymousSession: RequestHandler = (req, res, next) => {
  const user = req.user as any;
  if (req.isAuthenticated() && user?.anonymousToken) {
    (req as any).anonymousToken = user.anonymousToken;
  }
  next();
};